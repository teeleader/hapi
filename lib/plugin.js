...
internals.Plugin.prototype.register = function (plugins /*, [options], callback */) {
    ...
    /*
        ...
        register.attributes = {
            ...
            once: true,
            validate: {
                schema: Joi, true, false, func,
                options: Joi options,
                failAction: 'error' (default), 'log', 'ignore'
            }
        };
    ...
    */
    ...
    const each = (item, next) => {
        ...
        // Check option validation if provided
        // HELP: (not sure if this is where it should fall in the registration logic)

        if (item.register.attributes.validate) {

            const itemOptionsResult = Joi.validate(registrationData.options, registrationData.attributes.validate.schema, registrationData.attributes.validate.options);

            // failAction: 'error', 'log'

            if (itemOptionsResult.err) {

                if (registrationData.attributes.validate.failAction === 'error') {
                    return next(itemOptionsResult.err);
                }
                else if (registrationData.attributes.validate.failAction === 'log') {
                    // how do we server log here and track the err?
                }
            }

            // override the item pluginOptions with our Joi validated value

            item.pluginOptions = itemOptionsResult.value;
        }
